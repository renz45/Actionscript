package com.Fullsail.video
{
	...
	{
		private var _vidHeight:Number = 166;
		private var _vidWidth:Number = 250;
		...
		private function init():void
		{
			...
			setupNS();//we call this directly since the netconnection is being established by the main class
		}//end init
		...
		//this function is unused since the netConnection should be created outside this class, it is here for reference only.
		...
		private function ns_statusHandler(event:NetStatusEvent):void
		{
			...
			{
				var ev:VideoPlayerEvent = new VideoPlayerEvent(VideoPlayerEvent.PUBLISH_START,true);
				this.dispatchEvent(ev);
			}
			
			if(event.info.code == "NetStream.Unpublish.Success")
			{
				var ev2:VideoPlayerEvent = new VideoPlayerEvent(VideoPlayerEvent.PUBLISH_STOP,true);
		...
		public function record(streamName:String,type:String):void
		{
			if(_ns)
			{
				...
				this.width = _vidWidth;
				this.height = _vidHeight;
				...
				trace("the netStream doesnt exist in videoPlayer.as")
			}
		}//end record
